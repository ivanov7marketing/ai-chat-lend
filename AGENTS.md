...
- **Оптимизация команд (CRITICAL)**: В корне проекта (`C:\dev\ai-chat-lend`) инициализация шелла может тормозить git-команды.
- **Способ ускорения**: Запускать git-команды из подпапок (`frontend` или `backend`), используя флаг `-C ..`.
  - Пример: `cd frontend; git -C .. status`
...
## Git/деплой
- Среда выполнения: Windows PowerShell.
- **Cwd — ВСЕГДА корень проекта (CRITICAL)**:
  - `run_command` → `Cwd` = `c:\dev\ai-chat-lend` (корень workspace).
  - **Никогда** не ставить `Cwd` в подпапку (`frontend/`, `backend/` и т.д.) — команды будут молча отменены (баг Antigravity).
