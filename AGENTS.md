...
## Git/деплой
- Среда выполнения: Windows PowerShell. Оператор && НЕ работает в PowerShell.
- Git-команды выполнять ТОЛЬКО раздельно, каждую отдельной строкой:
    git add .
    git commit -m "feat: описание"
    git push
- Никогда не объединять команды через && или через точку с запятой в одну строку.
- После git push деплой на сервер происходит АВТОМАТИЧЕСКИ через GitHub Actions.
- Никогда не подключаться к серверу по SSH самостоятельно.
- Никогда не запускать docker compose на сервере вручную.
