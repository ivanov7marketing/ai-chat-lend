...
- **Cwd — ВСЕГДА корень проекта со СТРОЧНОЙ буквой диска и ПРЯМЫМИ слэшами (CRITICAL)**:
  - `run_command` → `Cwd` = `c:/dev/ai-chat-lend` (маленькая `c` и `/`).
  - **Никогда** не ставить `Cwd` в подпапку или использовать заглавную букву диска (`C:\`) — команды будут молча отменены.
  - Использование обратных слэшей в `Cwd` может приводить к ошибкам, используйте `/`.
