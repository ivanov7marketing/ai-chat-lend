...
- **Cwd — ВСЕГДА корень проекта со СТРОЧНОЙ буквой диска (CRITICAL)**:
  - `run_command` → `Cwd` = `c:\dev\ai-chat-lend` (обязательно маленькая `c`).
  - **Никогда** не ставить `Cwd` в подпапку или использовать заглавную букву диска (`C:\`) — команды будут молча отменены (баг Antigravity).
